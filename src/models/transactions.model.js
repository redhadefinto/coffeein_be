...
const getDetailHistory = (id, tpsId) => {
  return new Promise((resolve, reject) => {
    const sql = `SELECT u.email, tps.transactions_id, p.product_name, p.image, p.price, s."size", pr.code, py."method" AS "payment_method", st."name" AS "status", tps.quantity, tps.subtotal, TO_CHAR(t.created_at, 'YYYY-MM-DD') AS created_at 
      FROM transactions_products_sizes tps
      JOIN transactions t ON t.id = tps.transactions_id 
      JOIN products p ON p.id = tps.product_id
      JOIN sizes s ON s.id = tps.size_id
...
const getAllTransaction = () => {
  return new Promise((resolve, reject) => {
    const sql = `SELECT u.id as user_id, tps.transactions_id, p.id as product_id, s.id as size_id, p.product_name, p.image, p.price, s."size", pr.code, py."method" AS "payment_method", st."name" AS "status", tps.quantity, tps.subtotal, TO_CHAR(t.created_at, 'YYYY-MM-DD') AS created_at
      FROM transactions_products_sizes tps
      JOIN transactions t ON t.id = tps.transactions_id 
      JOIN products p ON p.id = tps.product_id
      JOIN sizes s ON s.id = tps.size_id
